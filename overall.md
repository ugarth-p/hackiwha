...
Agent 2: Analysis + Strategy Agent
        |
        v
Agent 3: Monitoring Agent  ---(on schedule)---> loops back to Agent 1a + 1b
        |
        v (produces a concept/strategy artifact to validate)
[PARALLEL, N agents]
  Agent 4.1..4.N: Persona Agents (Parent, Gen Z, Loyal fan, ...custom)
        |
        v (round 2+)
Agent 5: Panel Debate (personas re-respond to each other's round-1 output)
        |
        v
Agent 6: Moderator Agent
        |
...
create table pipeline_steps (
  ...
  step_name text not null, -- 'market_intel' | 'competitor_recon' | 'analysis_strategy' | 'monitoring' | 'persona_<name>' | 'panel_debate' | 'moderator' | 'report'
...
### Phase 3 — Analysis + Strategy agent (merged)

- Input: outputs of Agent 1a (market_intelligence) and Agent 1b (competitor_recon), plus (on 2nd+ runs) the previous run's output for comparison, plus a static "pattern library" (a JSON or markdown file you maintain, listing known growth tactics: psychological pricing, premiumization, subscription conversion, viral referral loops, etc. — seed with 10–15 entries to start).
- No tools required — pure reasoning over the two research inputs.
- Directive: do TWO jobs in one response, in order:
  - **Phase A — Analysis:** merge market context and competitor specifics into one structured competitive intelligence report. Explicitly call out opportunities (gaps competitors aren't covering) and threats (competitor moves that could hurt the tenant). Include a per-competitor comparison (strengths, weaknesses), a market_position_summary, and a SWOT.
  - **Phase B — Strategy:** recommend which patterns from the pattern library apply here and why, plus concrete pricing/marketing/roadmap recommendations. Ground every recommendation in a specific opportunity or threat from your own Phase A analysis — no generic advice.
- Validation rules (enforce in code before writing to `pipeline_steps`):
  - Every `pattern` must match a name from the provided pattern library.
  - Every `cites` must reference the `title` of an opportunity or threat from your own Phase A analysis.
  - On violation: retry once with the error appended; if it fails twice, mark the step `failed` and halt the run.
- Output schema (one combined JSON object):
```json
{
  "analysis": {
    ...
    "competitor_gaps": [{"competitor": "string", "gap": "string"}]
  },
  "strategy": [
    {
      "pattern":         "string — must match a pattern name from the library",
      "description":     "string",
      "cites":           "string — must reference an opportunity/threat title from analysis",
      "expected_impact": "string"
    }
  ]
}
```
- Write the combined output to `pipeline_steps` with `step_name = 'analysis_strategy'`. This single row is what the Monitoring Agent (next phase) reads as the strategy artifact for the run, and what the dashboard renders.
...
```
- `concept_for_validation` is whatever needs human/persona judgment this cycle — e.g. a new pricing recommendation, a proposed positioning change. This is what feeds Phase 5.
...
### Phase 5 — Persona panel (parallel)
...
```json
{
  ...
  "score_by_segment": [ { "persona_name": "string", "score": 0, "reasoning": "string" } ]
}
```
...
## 5. Multi-variant support (optional, build after Phase 8 works for a single concept)

To support comparing multiple concepts (e.g. three logo/positioning options) with one panel run:
- Fan Phase 5 out across each variant: every persona reacts to every variant (N personas × M variants independent calls).
- Phase 6's debate transcript should be grouped by variant.
- Phase 7's moderator receives all variants' transcripts together and is explicitly asked to rank variants per segment, not just score each in isolation.
...
- The Monitoring Agent (Phase 4) does not need to programmatically re-trigger anything — the scheduler is the source of truth for cadence. Keep these decoupled.
...
- Keep each agent's system prompt scoped to its one job. Resist the urge to let personas see the moderator's scoring criteria (that would bias their reactions).
...
- [ ] Phase 3: Analysis + Strategy agent (merged, with pattern library + grounding validation)
- [ ] Phase 4: Monitoring agent + delta detection
- [ ] Phase 5: Persona agents (start with the 3 defaults), round 1 only
- [ ] Phase 6: Panel debate, round 2+
- [ ] Phase 7: Moderator agent + scoring dashboard view
- [ ] Phase 8: Report generator
