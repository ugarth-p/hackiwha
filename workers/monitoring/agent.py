...
def _flatten_competitors(competitor_data: Any) -> dict[str, Any]:
    if isinstance(competitor_data, dict):
        if "competitors" in competitor_data and isinstance(
            competitor_data["competitors"], list
        ):
            names = []
            pricing = []
            positioning = []
            for c in competitor_data["competitors"]:
                if isinstance(c, dict):
                    ...
                    pricing.append(c.get("pricing_notes", ""))
                    positioning.append(c.get("positioning", ""))
...
def _flatten_market_intel(market_data: Any) -> dict[str, Any]:
    if isinstance(market_data, dict):
        result: dict[str, Any] = {}
        if "market_trends" in market_data:
            result["market_trends"] = market_data["market_trends"]
        if "typical_pricing_models" in market_data:
            result["pricing_models"] = market_data["typical_pricing_models"]
        if "common_customer_pain_points" in market_data:
            result["customer_pain_points"] = market_data[
...
def _prepare_comparison_data(
    ...
    market = step_outputs.get("market_intelligence", {})
    if market:
        merged.update(_flatten_market_intel(market))

    competitor = step_outputs.get("competitor_recon", {})
    if competitor:
        merged.update(_flatten_competitors(competitor))

    strategy = step_outputs.get("strategy_output", {})
    if strategy:
        merged["strategy_output"] = strategy
...
def _rule_based_comparison(
    ...
    pricing_changes = _compute_list_delta(
        current_pricing, previous_pricing, "pricing model"
    )
    ...
    trend_changes = _compute_list_delta(
        current_trends, previous_trends, "market trend"
    )
    if trend_changes:
        significant = True
    ...
    pain_changes = _compute_list_delta(
        current_pain, previous_pain, "pain point"
    )
    if pain_changes:
        significant = True
